...
const SpinningGlobe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Simplified continent outlines in lat/lon
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = 'hsla(38, 80%, 50%, 0.25)';
      ctx.lineWidth = 0.8;
      ctx.stroke();

      // Inner fill
      const innerGrad = ctx.createRadialGradient(cx - r * 0.25, cy - r * 0.25, 0, cx, cy, r);
      innerGrad.addColorStop(0, 'hsla(38, 80%, 55%, 0.04)');
      innerGrad.addColorStop(1, 'hsla(38, 80%, 55%, 0.01)');
      ...
      for (let lat = -60; lat <= 60; lat += 30) {
        ...
        ctx.strokeStyle = 'hsla(38, 80%, 50%, 0.1)';
        ctx.lineWidth = 0.4;
        ctx.stroke();
      }
      ...
      continents.forEach((continent) => {
        ...
        ctx.strokeStyle = 'hsla(38, 80%, 55%, 0.35)';
        ctx.lineWidth = 0.7;
        ctx.stroke();
        
        // Fill visible continents
        ctx.fillStyle = 'hsla(38, 80%, 55%, 0.04)';
        ctx.fill();
      });

      // Small dots at grid intersections (only visible side)
      for (let lon = 0; lon < 360; lon += 60) {
        ...
        for (let lat = -60; lat <= 60; lat += 30) {
          ...
          ctx.fillStyle = `hsla(38, 80%, 55%, ${0.2 + z3d * 0.3})`;
          ctx.fill();
        }
      }
